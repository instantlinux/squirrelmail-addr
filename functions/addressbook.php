 ...
 *
 * $Id: addressbook.php,v 1.9 2010/11/09 00:24:46 richb Exp $
 */
    ...
    */
    if (isset($addrbook_dsn) && !empty($addrbook_dsn)) {
        ...
        $r = $abook->add_backend('database', Array('dsn' => $addrbook_dsn,
                            'owner' => $username,
                            'table' => $addrbook_table.'_'.$username,
			    'history' => 'history_'.$username,
...
*/        
     ...
     */
    function category_count($category, $bnum) {
        ...
        /* Invoke addrecs in backend */
        return $this->backends[$bnum]->category_count($category);
    }

    /*
     * Add a new category
     */
    function category_create($category, $bnum) {
        ...
        /* Invoke category_add in backend */
        $res = $this->backends[$bnum]->category_create($category);
        if ($res) {
            return $bnum;
        } else {
            $this->error = $this->backends[$bnum]->error;
        ...
        return FALSE;  /* Not reached */
    } /* end of category_create() */

    /*
     * Remove categories
     */
    function category_drop($catlist, $bnum) {
        ...
        /* Invoke category_add in backend */
        $res = $this->backends[$bnum]->category_drop($catlist);
        if ($res) {
            return $bnum;
        } else {
            $this->error = $this->backends[$bnum]->error;
        ...
        return FALSE;  /* Not reached */
    } /* end of category_drop() */

    /*
     * Add records to category
     */
    function category_addrecs($category, $uids, $bnum) {
        ...
        /* Invoke addrecs in backend */
        $res = $this->backends[$bnum]->category_addrecs($category, $uids);
        if ($res) {
            return $bnum;
        } else {
            $this->error = $this->backends[$bnum]->error;
        ...
        return FALSE;  /* Not reached */
    } /* end of category_addrecs() */

    /*
     * Move records to new category
     */
    function category_move($category, $oldcategory, $bnum) {
        ...
        /* Invoke addrecs in backend */
        $res = $this->backends[$bnum]->category_move($category, $oldcategory);
        if ($res) {
            return $bnum;
        } else {
            $this->error = $this->backends[$bnum]->error;
        ...
        return FALSE;  /* Not reached */
    } /* end of category_move() */

    /*
     * Add records to category
     */
    function category_rmrecs($category, $uids, $bnum) {
        ...
        /* Invoke addrecs in backend */
        $res = $this->backends[$bnum]->category_rmrecs($category, $uids);
        if ($res) {
            return $bnum;
        } else {
            $this->error = $this->backends[$bnum]->error;
        ...
        return FALSE;  /* Not reached */
    } /* end of category_rmrecs() */
    
    /*
     * Get record history
     */
    function get_history($uid, $bnum) {
        ...
        /* Invoke addrecs in backend */
        return $this->backends[$bnum]->get_history($uid);
    }

    /*
     * Get record history
     */
    function raw_sql_cmd($query, $bnum) {
        ...
        /* Invoke addrecs in backend */
        return $this->backends[$bnum]->raw_sql_cmd($query);
    }
...
function alistcmp($a,$b) {
    ...
    /* changed sort sequence to use lastname, not fullname */
